...
watchEffect(() => {
  ...
  useDraggableBlockMenu(editor, props.anchorElem, editor._editable);
})
</script>
<template>
  <Teleport v-if="anchorElem" :to="anchorElem">
    <div
        class="icon draggable-block-menu"
        ref="menuRef"
        draggable
        @dragstart="onDragStart"
        @dragend="onDragEnd">
        <div :class="{'icon' : isEditable}"></div>
      </div>
      <div class="draggable-block-target-line" ref="targetLineRef"></div>
    </Teleport>
</template>
<style>
...
.draggable-block-menu .icon {
  ...
  height: 16px;
  opacity: 0.3;
  background-image: url('@/lexical/assets/images/icons/draggable-block-menu.svg');
}
